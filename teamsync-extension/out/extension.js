...
function activate(context) {
    ...
    app.get('/git-status', async (req, res) => {
        ...
        try {
            ...
            const lastFetch = context.globalState.get(fetchKey) || '';
            const lastPull = context.globalState.get(pullKey) || '';
            const lastPush = context.globalState.get(pushKey) || '';
            res.json({
                exists: true,
                isGit: true,
                path: targetDir,
